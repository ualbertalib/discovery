...
module Blacklight::ArticlesHelperBehavior
  ...
  def show_hidden_field_tags
    ...
    params.each do |key, value|
      unless (key == 'search_field') || (key == 'fromDetail') || (key == 'facetfilter') || (key == 'pagenumber') || (key == 'q') || (key == 'dbid') || (key == 'an')
        if key == 'eds_action'
          if !value.scan(/addlimiter/).empty? || !value.scan(/removelimiter/).empty? || !value.scan(/setsort/).empty? || !value.scan(/SetResultsPerPage/).empty?
      ...
      if result['FullText']['Text'].present?
        if result['FullText']['Text']['Availability'].present?
          if result['FullText']['Text']['Availability'] == '1'
            HTMLEntities.new.decode(result['FullText']['Text']['Value'])
          end
        end
      end
    end
  ...
  def show_pdf_title_link(result)
    title_pdf_link = ''
    if result['Header']['DbId'].present? && result['Header']['An'].present?
