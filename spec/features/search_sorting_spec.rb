...
RSpec.describe 'Search sorting', type: :feature do
  scenario 'User can sort by year' do
    ...
    expect(first('#documents .document')).to have_text('William Shakespeare, the complete works, Original-spelling ed.')
  end
  ...
  scenario 'User can sort by author' do
    ...
    expect(first('#documents .document')).to have_text('Shakespeare at the Globe : 1599-1609')
  end
  ...
  scenario 'User can sort by title' do
    ...
    expect(first('#documents .document')).to have_text('Amazing monument : a short history of the Shakespeare industry')
  end
end
