describe "ingest rake tasks" do
  ...
  it 'use url from blacklight config should print to screen and run ingest task successfully' do
    ...
    expect { Rake::Task["ingest_info"].invoke }.to output("WARNING: Using live target from 'test' stanza in config/blacklight.yml (http://localhost:8983/solr/discovery-test)\nSolr collection contains 10024 results.\n").to_stdout
    expect { Rake::Task['ingest'].invoke('symphony_test_set') }.to_not raise_error SystemExit
    ENV['SOLR_INGEST_URL'] = old_env_var
  end
end
